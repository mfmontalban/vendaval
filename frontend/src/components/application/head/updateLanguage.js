  ...
  render(){
    ...
    return(
      <div ref={this.setWrapperRef} className="d-flex flex-direction-column justify-content-right">
        <Button onClick={() => this.toggleList()} className="border-0 text-right clickable" transitionStyled={`${application.transitions.general}`} backgroundStyled={`${application.settings.themeTransparent}`} colorStyled={`${application.mode.primary}`} colorHoverStyled={`${application.theme.primary}`} paddingStyled={`${application.settings.appPadding}`} radiusStyled={`${application.settings.appRadius}`}>
          <span className="p-5px">{application.language}</span><i className="fas fa-caret-down"></i>
        </Button>
        {listOpen && 
          <Dropdown className="mt-40px ml-neg25px position-absolute z-1005 d-flex flex-direction-column text-right outer-shadow" transitionStyled={`${application.transitions.general}`} backgroundStyled={`${application.mode.primary}`} colorStyled={`${application.theme.primary}`} radiusStyled={`${application.settings.appRadius}`}>
