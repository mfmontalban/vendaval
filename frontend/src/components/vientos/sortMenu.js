      ...
      <div ref={this.setWrapperRef} className="mt-neg43px">
        ...
        {listOpen && 
          <Div className="position-absolute z-1005 d-flex flex-direction-column text-right outer-shadow-primary border-1 ml-neg107px" backgroundStyled={`${application.mode.primary}`} backgroundHoverStyled={`${application.mode.primary}`} colorStyled={`${application.theme.primary}`} radiusStyled={`${application.settings.appRadius}`}>
            ...
            <Button onClick={(e) => {this.props.setSortsTitleDown(e); this.toggleList();}} className="p-10px text-right" backgroundStyled={`${application.mode.primary}`} backgroundHoverStyled={`${application.theme.primaryQuarter}`} colorStyled={`${application.theme.primary}`} type="Button">{titleDown}</Button>
            <Button onClick={(e) => {this.props.setSortsRecentNewest(e); this.toggleList();}} className="p-10px text-right" backgroundStyled={`${application.mode.primary}`} backgroundHoverStyled={`${application.theme.primaryQuarter}`} colorStyled={`${application.theme.primary}`} type="Button">{recent}</Button>
            <Button onClick={(e) => {this.props.setSortsRecentOldest(e); this.toggleList();}} className="p-10px bottom-border-radius text-right" backgroundStyled={`${application.mode.primary}`} backgroundHoverStyled={`${application.theme.primaryQuarter}`} colorStyled={`${application.theme.primary}`} type="Button">{older}</Button>
          </Div>
        }
      </div>
    )
